...
class DatabaseManager {
  ...
  /**
   * Initialize SQLite with connection pooling
   * SECURITY FIX: Fail-fast in production if Postgres is not available
   */
  async initializeSQLite() {
    // SECURITY FIX: In production, SQLite should not be used
    // All data should be in Postgres for persistence
    const isProduction = process.env.NODE_ENV === 'production';
    const allowSqliteFallback = process.env.ALLOW_SQLITE_FALLBACK === '1' || 
                                (!isProduction && process.env.ALLOW_SQLITE_FALLBACK !== '0');
    
    if (isProduction && !allowSqliteFallback) {
      ...
      if (!hasPostgres) {
        throw new Error(
          ...
          'Please configure POSTGRES_PRISMA_URL or set ALLOW_SQLITE_FALLBACK=1 (not recommended). ' +
          'SQLite data is ephemeral in serverless environments and will be lost.'
        );
      }
      
      // Even if Postgres is configured, warn if SQLite is being used
      console.warn('⚠️ WARNING: SQLite initialization attempted in production. ' +
                   'All gaming endpoints should use Postgres via requirePostgres().');
    }
    ...
    try {
      ...
      // Note: /tmp is ephemeral and will be cleared between invocations
      let dataDir;
      if (isServerless) {
        dataDir = '/tmp/data';
        console.log("⚠️ Serverless environment detected - using /tmp for database (ephemeral storage)");
        if (isProduction) {
          throw new Error(
            'SQLite in /tmp is not allowed in production serverless environments. ' +
            'Data will be lost between invocations. Use Postgres instead.'
          );
        }
      } else {
      ...
      if (isServerless) {
        console.log("✅ SQLite database initialized in /tmp (serverless mode - DEVELOPMENT ONLY)");
      } else {
        console.log("✅ SQLite database initialized with connection pooling (DEVELOPMENT ONLY)");
      }
    } catch (error) {
